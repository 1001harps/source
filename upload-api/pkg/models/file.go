...
func (r *fileR) GetTenantIdTenant() *Tenant {
	if r == nil {
		return nil
	}
...
func (fileL) LoadTenantIdTenant(ctx context.Context, e boil.ContextExecutor, singular bool, maybeFile interface{}, mods queries.Applicator) error {
	var slice []*File
	var object *File
	...
	query := NewQuery(
		qm.From(`tenant`),
		qm.WhereIn(`tenant.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		...
		foreign.R.TenantIdFiles = append(foreign.R.TenantIdFiles, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.TenantId == foreign.ID {
				...
				foreign.R.TenantIdFiles = append(foreign.R.TenantIdFiles, local)
				break
			}
		}
	}
...
// SetTenantIdTenant of the file to the related item.
// Sets o.R.TenantIdTenant to related.
// Adds o to related.R.TenantIdFiles.
func (o *File) SetTenantIdTenant(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Tenant) error {
	...
	if related.R == nil {
		related.R = &tenantR{
			TenantIdFiles: FileSlice{o},
		}
	} else {
		related.R.TenantIdFiles = append(related.R.TenantIdFiles, o)
	}
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(filePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert file, could not build conflict column list")
			}
