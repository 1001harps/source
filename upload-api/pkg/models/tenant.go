...
var TenantWhere = struct {
	...
	ApiKey:   whereHelperstring{field: "\"tenant\".\"apiKey\""},
	Name:     whereHelperstring{field: "\"tenant\".\"name\""},
	Created:  whereHelpertime_Time{field: "\"tenant\".\"created\""},
	Modified: whereHelpertime_Time{field: "\"tenant\".\"modified\""},
	Active:   whereHelperbool{field: "\"tenant\".\"active\""},
}
...
type tenantR struct {
	TenantIdFiles FileSlice `boil:"TenantIdFiles" json:"TenantIdFiles" toml:"TenantIdFiles" yaml:"TenantIdFiles"`
}
...
func (r *tenantR) GetTenantIdFiles() FileSlice {
	if r == nil {
		return nil
	}
	return r.TenantIdFiles
}
...
var (
	tenantAllColumns            = []string{"id", "apiKey", "name", "created", "modified", "active"}
	tenantColumnsWithoutDefault = []string{"name"}
	tenantColumnsWithDefault    = []string{"id", "apiKey", "created", "modified", "active"}
...
func (o *Tenant) TenantIdFiles(mods ...qm.QueryMod) fileQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"file\".\"tenantId\"=?", o.ID),
	)

	return Files(queryMods...)
}

// LoadTenantIdFiles allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (tenantL) LoadTenantIdFiles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTenant interface{}, mods queries.Applicator) error {
	var slice []*Tenant
	var object *Tenant
	...
	query := NewQuery(
		qm.From(`file`),
		qm.WhereIn(`file.tenantId in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.TenantIdFiles = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.TenantId {
				local.R.TenantIdFiles = append(local.R.TenantIdFiles, foreign)
				if foreign.R == nil {
					foreign.R = &fileR{}
				}
...
// AddTenantIdFiles adds the given related objects to the existing relationships
// of the tenant, optionally inserting them as new records.
// Appends related to o.R.TenantIdFiles.
...
func (o *Tenant) AddTenantIdFiles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*File) error {
	...
	if o.R == nil {
		o.R = &tenantR{
			TenantIdFiles: related,
		}
	} else {
		o.R.TenantIdFiles = append(o.R.TenantIdFiles, related...)
	}
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(tenantPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert tenant, could not build conflict column list")
			}

			conflict = make([]string, len(tenantPrimaryKeyColumns))
			copy(conflict, tenantPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"tenant\"", updateOnConflict, ret, update, conflict, insert, opts...)
